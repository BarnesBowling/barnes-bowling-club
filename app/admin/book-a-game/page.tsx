  ...
  return (
    <div ref={wrapRef} style={{ position: 'relative', ...style }}>
      ...
      {open && (
        ...
        >
          {filtered.length === 0 ? (
            <li style={{ padding: '10px 12px', fontFamily: "'DM Sans', sans-serif", fontSize: '13px', color: 'var(--text-muted)', fontStyle: 'italic' }}>
              No matches
            </li>
          ) : filtered.map((name, i) => (
            <li
...
function FixturesCalendar({ selectedDate, refreshKey }: { selectedDate: string; refreshKey: number }) {
  ...
  return (
    <div style={{ border: '1.5px solid var(--green-deep)', position: 'sticky', top: '1.5rem', maxHeight: 'calc(100vh - 4rem)', display: 'flex', flexDirection: 'column', background: 'var(--cream, #ffffff)' }}>
      ...
      <div style={{ overflowY: 'auto', flex: 1 }}>
        {loading ? (
          <div style={{ padding: '2rem 1rem', fontFamily: "'DM Sans', sans-serif", fontSize: '12px', color: 'var(--text-muted)', textAlign: 'center' }}>Loading fixtures…</div>
        ) : (
          days14.map((day, i) => {
            ...
            return (
              <div key={day} style={{ padding: '11px 16px', borderBottom: i < 13 ? '1px solid rgba(45,90,61,.08)' : 'none', borderLeft: isSelected ? '3px solid var(--green-deep)' : '3px solid transparent', background: isSelected ? 'rgba(45,90,61,.05)' : 'transparent' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '6px' }}>
                  <span style={{ fontFamily: "'DM Sans', sans-serif", fontSize: '12px', fontWeight: 700, color: isSelected ? 'var(--green-deep)' : 'var(--text-dark)', textTransform: 'uppercase', letterSpacing: '.04em' }}>{dateLabel}</span>
                ...
                {dayBookings.map(b => {
                  ...
                  return (
                    <div key={b.id} style={{ display: 'flex', gap: '6px', alignItems: 'baseline', marginTop: '4px', flexWrap: 'wrap' }}>
                      <span style={{ fontFamily: "'DM Sans', sans-serif", fontSize: '11px', fontWeight: 700, color: 'var(--green-deep)', minWidth: '2.75rem', flexShrink: 0 }}>{b.time_slot}</span>
                      <span style={{ fontFamily: "'DM Sans', sans-serif", fontSize: '9px', fontWeight: 700, letterSpacing: '.07em', textTransform: 'uppercase', color: colors.text, background: colors.bg, padding: '1px 6px', whiteSpace: 'nowrap', flexShrink: 0 }}>{compLabel(b.competition)}</span>
                      <span style={{ fontFamily: "'DM Sans', sans-serif", fontSize: '11px', color: 'var(--text-mid)', lineHeight: 1.4 }}>{formatBookingPlayers(b)}</span>
                    </div>
                  );
                })}
              </div>
...
export default function AdminBookAGamePage() {
  ...
  const loadAllBookings = useCallback(async () => {
    const end = localDateStr(new Date(Date.now() + 90 * 24 * 60 * 60 * 1000));
    const range = await getBookingsForRange(today, end);
    setAllBookings(range);
  }, [today]);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSubmitting(true);
    const result = await createFixtureBookingAsAdmin({
      competition,
    ...
    <div style={{ padding: '2rem', maxWidth: '1200px', margin: '0 auto' }}>
      ...
      <div className="book-game-layout">
        <div>
              ...
              {date && (
                <div style={{ marginBottom: '2rem' }}>
                  <div style={{ display: 'flex', alignItems: 'baseline', gap: '0.75rem', marginBottom: '1rem' }}>
                    ...
                    {loadingSlots && <span style={{ fontFamily: "'DM Sans', sans-serif", fontSize: '11px', color: 'var(--text-muted)' }}>checking availability…</span>}
                  </div>
          ...
          {mode === 'cancel' && (
            <div>
              <div style={{ ...sectionHeadStyle, marginBottom: '1.25rem' }}>All upcoming bookings</div>
              {allBookings.length === 0 ? (
                <p style={{ fontFamily: "'Libre Baskerville', serif", fontSize: '14px', color: 'var(--text-muted)', fontStyle: 'italic' }}>No upcoming bookings found.</p>
              ) : (
                      ...
                      <div key={b.id} style={{ background: 'var(--cream)', padding: '1rem 1.25rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '1rem', flexWrap: 'wrap' }}>
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '4px', flex: 1, minWidth: 0 }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
                            <span style={{ fontFamily: "'DM Sans', sans-serif", fontSize: '9px', fontWeight: 700, letterSpacing: '.1em', textTransform: 'uppercase', color: colors.text, background: colors.bg, padding: '2px 7px' }}>{compLabel(b.competition)}</span>
                            <span style={{ fontFamily: "'DM Sans', sans-serif", fontSize: '12px', fontWeight: 700, color: 'var(--green-deep)' }}>{dateLabel} · {b.time_slot}</span>
                          </div>
                          <span style={{ fontFamily: "'Libre Baskerville', serif", fontSize: '13px', color: 'var(--text-mid)' }}>{formatBookingPlayers(b)}</span>
                          <span style={{ fontFamily: "'DM Sans', sans-serif", fontSize: '11px', color: 'var(--text-muted)' }}>{b.member_email}</span>
                        </div>
                        <button
                          type="button"
