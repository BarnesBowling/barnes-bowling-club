...
export async function POST(req: Request) {
  ...
  const { title, firstName, lastName, company, phone, email, message } = parsed.data;
...
<body style="margin:0;padding:0;background:#f0ede6;font-family:Arial,Helvetica,sans-serif">
          ...
          <td style="background:#fff;padding:36px">
            <table width="100%" cellpadding="0" cellspacing="0" style="font-size:14px;line-height:1.6;color:#1a2e1f;border-collapse:collapse">
              ...
              <tr>
                ...
                <td style="padding:10px 0 10px 16px;border-bottom:1px solid #eae8e2">${company}</td>
              </tr>` : ''}
              <tr>
  ...
  if (emailError) {
    ...
    return Response.json({ error: 'Failed to send your message. Please try again or email us directly.' }, { status: 500 });
  }
