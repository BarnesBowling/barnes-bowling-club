...
export type MemberData = {
  ...
  firstname: string;
  surname: string;
  h: { [year: number]: number | undefined };
};
...
function getTrend(h: { [year: number]: number | undefined }): { symbol: string; color: string } {
  const h25 = h[2025];
  const h26 = h[2026];
...
export function HandicapsClient({ members }: { members: MemberData[] }) {
  const [sortMode, setSortMode] = useState<SortMode>('handicap');

  const sortedMembers = useMemo(() => {
    if (sortMode === 'handicap') {
      return [...members].sort((a, b) => {
        const ah = a.h[2026] ?? 999;
        const bh = b.h[2026] ?? 999;
        const diff = ah - bh;
    ...
    <>
      ...
      <main>
        ...
        <div className="section-inner" style={{ padding: '3rem 2rem 5rem' }}>
                ...
                {sortedMembers.map((m, i) => {
                  ...
                  return (
                    <tr key={`${m.surname}-${m.firstname}`} style={{
                      ...
                      <td style={{ padding: '11px 10px', fontSize: '15px', color: 'var(--text-dark)', fontWeight: isTop3 ? 500 : 400, whiteSpace: 'nowrap' }}>
                      ...
                      <td style={{ padding: '11px 10px', textAlign: 'center', fontSize: '13px', color: t.color, fontFamily: "'DM Sans', sans-serif" }}>
                        {t.symbol}
                      </td>
                    </tr>
                  );
          ...
          {/* Key */}
          <div style={{ marginTop: '1.25rem', display: 'flex', gap: '1.5rem', flexWrap: 'wrap', fontFamily: "'DM Sans', sans-serif", fontSize: '11px', color: 'var(--text-muted)', letterSpacing: '.04em' }}>
            {[
          ...
          {/* About handicaps */}
          <div style={{ marginTop: '3rem', padding: '1.5rem 2rem', background: 'rgba(45,90,61,.04)', borderLeft: '3px solid rgba(45,90,61,.2)' }}>
            <div style={{ fontFamily: "'Playfair Display', serif", fontSize: '14px', fontWeight: 500, color: 'var(--green-deep)', marginBottom: '8px' }}>
          ...
          <div style={{ marginTop: '4rem', borderTop: '1.5px solid rgba(45,90,61,.12)', paddingTop: '2.5rem' }}>
            <div style={{ fontFamily: "'Playfair Display', serif", fontSize: '20px', fontWeight: 500, color: 'var(--green-deep)', marginBottom: '1.25rem' }}>
              Handicap Committee
            </div>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '1px', background: 'rgba(45,90,61,.08)' }}>
              {['Alaric Evans', 'Mark Hunter', 'Judith Heaton', 'Catherine Mitrenas', 'Toby Steedman'].map(name => (
                <div key={name} style={{ background: 'var(--cream)', padding: '1rem 1.25rem', minWidth: '160px', flex: '1 0 160px' }}>
                  <div style={{ fontFamily: "'DM Sans', sans-serif", fontSize: '9px', fontWeight: 600, letterSpacing: '.16em', textTransform: 'uppercase', color: 'var(--gold)', marginBottom: '4px' }}>Handicap Committee</div>
